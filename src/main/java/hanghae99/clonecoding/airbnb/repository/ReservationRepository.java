...
*/
    ...
    @Query(nativeQuery = true,
            value = "Select t1.check_out_date,t2.check_in_date " +
                    "from airbnb.reservation t1 " +
                    "left join airbnb.reservation t2 on t2.check_in_date = (select min(this.check_in_date) " +
                    "from airbnb.reservation this " +
                    "where this.check_in_date > t1.check_out_date and this.hotel_id = t1.hotel_id) " +
                    "where t1.hotel_id=:hotel and :date+2 < DATEDIFF(t2.check_in_date,t1.check_out_date) limit 1")
...
//            value = "SELECT T1.CHECK_OUT_DATE,T2.CHECK_IN_DATE " +
//                    "FROM RESERVATION T1 " +
//                    "LEFT JOIN RESERVATION T2 ON T2.CHECK_IN_DATE = (SELECT MIN(THIS.CHECK_IN_DATE) " +
//                    "FROM RESERVATION THIS " +
//                    "WHERE THIS.CHECK_IN_DATE > T1.CHECK_OUT_DATE AND THIS.HOTEL_ID = T1.HOTEL_ID) " +
//                    "WHERE T1.HOTEL_ID=:hotel AND :date+2 < DATEDIFF(DAY,T2.CHECK_IN_DATE,T1.CHECK_OUT_DATE) LIMIT 1")
